...
const deriveJobStatus = (job) => {
  ...
  if (job?.approvalStatus && job.approvalStatus !== "approved") return "draft";
...
export const postJob = async (req, res) => {
  ...
  if (!title || !description || !location || !level || !category || (!saveAsDraft && !deadline)) {
    return res.status(400).json({
      success: false,
      message:
  ...
  const parsedDeadline = deadline ? new Date(deadline) : new Date(Date.now() + 30 * 24 * 60 * 60 * 1000);
  if (Number.isNaN(parsedDeadline.getTime()) || (!saveAsDraft && parsedDeadline <= new Date())) {
    return res.status(400).json({
      success: false,
  ...
  try {
    ...
    const newJob = await Job({
      ...
      description,
      location,
      salary: saveAsDraft ? 0 : legacySalary,
      salaryMode,
      salaryAmount: saveAsDraft ? null : normalizedSalaryAmount,
      salaryMin: saveAsDraft ? null : normalizedSalaryMin,
      salaryMax: saveAsDraft ? null : normalizedSalaryMax,
      salaryVisible,
      isNegotiable: saveAsDraft ? true : isNegotiable,
      jobStatus: "draft",
  ...
  try {
    ...
    const jobsData = await Promise.all(
      jobs.map(async (job) => {
        ...
        const visible = job.approvalStatus === "approved" && status === "active" ? Boolean(job.visible) : false;
...
export const ChangeJobVisibility = async (req, res) => {
  try {
    ...
    const effectiveStatus = deriveJobStatus(job);
    if (job.approvalStatus !== "approved" || effectiveStatus !== "active") {
      return res.status(400).json({
        success: false,
        message: "Only approved active jobs can be made visible.",
      });
    }
...
export const moderateJobApproval = async (req, res) => {
  try {
    ...
    if (decision === "approved") {
      ...
      update.submittedForApprovalAt = null;
