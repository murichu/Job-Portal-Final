...
export const downloadCompanyReportExcel = async (req, res) => {
  try {
    ...
    csvLines.push(
      "Company Name,Office Location,Office Phone,Recruiter,Recruiter Position,Company Email,Logo URL"
    );
    csvLines.push(
      `"${company.name}","${company.companyLocation}","${
        company.companyPhone
      }","${company.recruiterName}","${company.recruiterPosition}","${
        company.email
      }","${company.image || ""}"`
    );
    csvLines.push("");
    ...
    csvLines.push(
      "Total Jobs,Total Applications,Longlisted,Shortlisted,Pending,Rejected"
    );
    csvLines.push(
    ...
    csvLines.push("Jobs");
    csvLines.push(
      "Title,Category,Level,Location,Salary Mode,Salary,Visible,Posted Date"
    );
    report.jobs.forEach((job) => {
      csvLines.push(
        `"${job.title}","${job.category}","${job.level}","${job.location}","${
          job.salaryMode || "fixed"
        }",${job.salary || 0},${job.visible ? "Yes" : "No"},"${new Date(
          job.date
        ).toLocaleDateString()}"`
      );
    });
    ...
    csvLines.push(
      "Candidate Name,Candidate Email,Job Title,Status,Applied Date"
    );
